...
namespace Shabasher.BusinessLogic.Services
{
    ...
    public class UsersManageService : IUsersManageService
    {
        ...
        public async Task<Result<UserRemoveAvatarResponse>> RemoveUserAvatarAsync(string userId)
        {
            var user = await _dbcontext.Users
                .Include(u => u.Participations)
                .ThenInclude(p => p.Shabash)
            ...
            if (user == null)
                return Result.Failure<UserRemoveAvatarResponse>("Пользователь не найден");
            ...
            var result = new UserRemoveAvatarResponse(UserResponseMapper.EntityToResponse(user), oldKey);

            return Result.Success<UserRemoveAvatarResponse>(result);
        }
